...
class PVLAggregate {
  ...
  get(key) {
    const foundItem = this.store.find((item) => item[0] === key);

    return foundItem ? foundItem[1] : null;
  }

  getAll(key) {
    return this.store.filter((item) => item[0] === key).map((item) => item[1]);
  }

  removeAll(key) {
    this.store = this.store.filter((item) => item[0] !== key);
  }
  ...
  objects(key) {
    return this.getAll('OBJECT').filter((o) => (key ? areIDsSame(o.identifier, key) : true));
  }

  groups(key) {
    return this.getAll('GROUP').filter((g) => (key ? areIDsSame(g.identifier, key) : true));
  }

  aggregates(key) {
    return this.objects(key).concat(this.groups(key));
  }
...
class PVLRoot extends PVLAggregate {
  ...
  toPVL() {
    return this.store.reduce((last, curr) => last.concat(`${curr[0]} = ${curr[1].toPVL()};\n`), '');
  }
}
